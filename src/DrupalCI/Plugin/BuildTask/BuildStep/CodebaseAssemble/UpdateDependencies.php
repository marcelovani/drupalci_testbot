...
class UpdateDependencies extends BuildTaskBase implements BuildStepInterface, BuildTaskInterface, Injectable {
  ...
  public function run() {
    ...
    if (in_array($contrib_dir . '/composer.json', $modified_files)) {
      ...
      $cmd = "composer${verbose} remove drupal/" . $project_name . " --ignore-platform-reqs --no-interaction --working-dir " . $source_dir;
      ...
      foreach($dev_dependencies as $package){
        ...
        $cmd = "composer${verbose} remove " . $dev_dep . " --no-interaction --ignore-platform-reqs --working-dir " . $source_dir;
        $this->io->writeln("Removing dev dependencies: $cmd");
...
git config --global user.name \"The Testbot\" && git commit -am 'intermediate commit'";
      ...
      // unset pdo
      $cmd = "composer${verbose} config repositories.pdo --unset --working-dir " . $source_dir;
      ...
      $cmd = "composer${verbose} config repositories.ancillary '{\"type\": \"path\", \"url\": \"" . $ancillary_dir . "\", \"options\": {\"symlink\": false}}' --working-dir " . $source_dir;
      ...
      $cmd = "composer${verbose} require drupal/" . $project_name . " 'dev-ancillary-branch as $composer_branchname' --ignore-platform-reqs --working-dir " . $source_dir;
      ...
      if (!empty($packages)) {
        $cmd = "composer${verbose} require " . implode(" ", $packages) . " --ignore-platform-reqs --prefer-stable${progress} --no-suggest --working-dir " . $source_dir;
