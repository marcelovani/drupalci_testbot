...
class YarnInstall extends BuildTaskBase {
  ...
  public function run() {
    ...
    $result = $this->execEnvironmentCommands("sudo -u www-data yarn${verbose} list{$progress} --non-interactive --cwd ${work_dir} > {$this->pluginWorkDir}/yarn_list.txt");
    $this->saveHostArtifact("{$this->pluginWorkDir}/yarn_list.txt", "yarn_list.txt");

    $result = $this->execEnvironmentCommands("sudo -u www-data yarn${verbose}{$progress} --non-interactive --cwd ${work_dir} licenses list > {$this->pluginWorkDir}/yarn_licenses.txt");
    $this->saveStringArtifact("{$this->pluginWorkDir}/yarn_licenses.txt", "yarn_licenses.txt");
